...
export const WebMenuAccessPage = ({ user }: { user: any }) => {
  ...
  const [newRtInput, setNewRtInput] = useState('');
  ...
  const handleAddRt = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newRtInput.trim()) return;
    ...
    try {
      const res = await apiFetch('/api/developer/rt', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ rtId: newRtInput.trim(), isVip: newRtVip })
      });

      const json = await res.json();
      if (res.ok) {
        setMessage({ text: json.message || 'Nomor RT berhasil ditambahkan', type: 'success' });
        setNewRtInput('');
        setNewRtVip(false);
        fetchRtList();
        fetchStats();
        ...
        setMessage({ text: json.error || 'Gagal menambahkan nomor RT', type: 'error' });
      }
    } catch (err) {
      console.error(err);
      setMessage({ text: 'Terjadi kesalahan server saat menambah RT', type: 'error' });
    } finally {
      setIsSubmittingRt(false);
    }
  };

  const handleToggleRtVip = async (rtId: string, currentVip: boolean) => {
    try {
      const res = await apiFetch(`/api/developer/rt/${rtId}/vip`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ isVip: !currentVip })
      });
  ...
  const handleDeleteRt = async (rtId: string) => {
    if (!window.confirm(`Apakah Anda yakin ingin menghapus nomor RT [${rtId.toUpperCase()}]?`)) return;

    try {
      const res = await apiFetch(`/api/developer/rt/${rtId}`, {
        method: 'DELETE'
      });
      const json = await res.json();
      if (res.ok) {
        setMessage({ text: json.message || `RT ${rtId} berhasil dihapus`, type: 'success' });
        fetchRtList();
        setTimeout(() => setMessage(null), 4000);
      } else {
        setMessage({ text: json.error || 'Gagal menghapus RT', type: 'error' });
      }
    } catch (err) {
      console.error(err);
      setMessage({ text: 'Gagal menghapus RT', type: 'error' });
    }
  };
  ...
  return (
    <div className="w-full min-h-[calc(100vh-100px)] p-6 bg-[#FAFBFD] space-y-6">
      ...
      {/* Kelola Daftar RT & Berlangganan VIP Panel */}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-gray-100 pb-4 gap-3">
          <div>
            <h3 className="text-base font-bold text-gray-800 flex items-center gap-2">
              ...
              Kelola Daftar RT Sistem & Lisensi VIP
            </h3>
            <p className="text-xs text-gray-500 mt-0.5">
              Tambah nomor RT baru ke dalam <code className="bg-gray-100 px-1.5 py-0.5 rounded text-indigo-600 font-mono text-[11px]">rtList</code> dan atur status berlangganan VIP secara sistemik.
            </p>
          </div>

          {/* Form Tambah RT Baru Inline */}
          <form onSubmit={handleAddRt} className="flex items-center gap-2 flex-wrap">
            <div className="relative">
              <input
                type="text"
                placeholder="Nomor RT (cth: 04 atau rt04)"
                value={newRtInput}
                onChange={(e) => setNewRtInput(e.target.value)}
                className="px-3.5 py-2 text-xs font-semibold bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-indigo-500 outline-none w-48"
                required
              />
            </div>
            <label className="flex items-center gap-1.5 cursor-pointer bg-gray-50 border border-gray-200 px-3 py-2 rounded-xl text-xs font-semibold text-gray-700 hover:bg-gray-100">
              <input
                type="checkbox"
                checked={newRtVip}
                onChange={(e) => setNewRtVip(e.target.checked)}
            ...
            <button
              type="submit"
              disabled={isSubmittingRt || !newRtInput.trim()}
              className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white font-bold text-xs rounded-xl shadow-sm flex items-center gap-1.5 disabled:opacity-50 transition-all cursor-pointer"
            >
              {isSubmittingRt ? (
          ...
          {rtList.map((r) => (
            ...
            >
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                <button
                  type="button"
                  onClick={() => handleToggleRtVip(r.rtId, r.isVip)}
